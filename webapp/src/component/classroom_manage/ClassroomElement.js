    ...
    return (
        <Box sx={{ border: 1, borderRadius: 2, padding: 1, marginBottom: 1 }} minWidth={195} textAlign="center">
            <Tooltip
                title={
                    <Stack spacing={1}>
                        <Typography variant="body2">{Resource.get('classes.min_grade')}: {Defines.gradeOptions.find(option => option.value === classItem.min_grade).label}</Typography>
                        <Typography variant="body2">{Resource.get('classes.max_grade')}: {Defines.gradeOptions.find(option => option.value === classItem.max_grade).label}</Typography>
            ...
            <TableContainer component={Paper}>
                <Table sx={{ minWidth: 175 }} aria-label="customized table">
                    <TableBody>
                        ...
                        <StyledTableRow sx={{}}>
                            <StyledTableCell align="center">{Resource.get('classroom.add_student')}</StyledTableCell>
                            <StyledTableCell align="center">
                                <IconButton
                                    aria-label="add"
                                    onClick={() => {
